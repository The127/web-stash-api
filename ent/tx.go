...
func (tx *Tx) init() {
	tx.Bag = NewBagClient(tx.config)
	tx.BagItem = NewBagItemClient(tx.config)
	tx.Share = NewShareClient(tx.config)
	tx.SubItem = NewSubItemClient(tx.config)
}
...
// applies a query, for example: Bag.QueryXXX(), the query will be executed
